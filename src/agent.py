...
class VLMNavAgent(Agent):
    ...
    def _is_stuck_in_small_area(self, goal_info=None):
        ...
        if len(self.memory_nodes) < 15:  # 至少需要15个记忆点才能判断
            return False
        ...
        for i in range(1, 16):  # 检查最近15个记忆节点
        ...
        if is_stuck:
            ...
            self.stuck_areas.append({
                ...
                'radius': max(max_distance, 1.5)  # 至少1.5米的避开半径
            })
            
            logging.warning(f"检测到小区域停滞! 活动范围:{max_distance:.2f}m, 净移动:{net_movement:.2f}m, 目标距离:{target_distance:.2f}m")
    ...
    def _generate_escape_from_area(self):
        ...
        if not closest_area or min_dist > 3.0:  # 如果已经远离所有卡住区域
            return PolarAction(1.0, np.random.uniform(-np.pi, np.pi))
        ...
        logging.info(f"生成远离卡住区域的动作: 距离={distance:.2f}m, 角度={np.degrees(escape_angle):.1f}°")
...
class ObjectNavAgent(VLMNavAgent):
    ...
    def _choose_action(self, obs: dict):
        ...
        # 然后检查是否需要激活精准模式（首次检测到目标）
            ...
            # 添加这个新检测
            stuck_in_area = False
            if self.step_ndx > 30:  # 至少等30步后再检测
                stuck_in_area = self._is_stuck_in_small_area(goal)
            
            if stuck_in_area:
                # 强制远离当前区域
                logging.warning(f"检测到小区域停滞，强制远离当前区域 (步骤: {self.step_ndx})")
                agent_action = self._generate_escape_from_area()
                step_metadata['action_number'] = 999  # 特殊标记表示强制远离
                ...
                # 检查是否检测到常规卡住状态
                if hasattr(self, '_is_agent_stuck') and self._is_agent_stuck():
                    logging.warning("检测到常规卡住状态，使用逃脱策略")
                    if hasattr(self, '_get_escape_action'):
                        agent_action = self._get_escape_action()
                        logging_data['STUCK_ESCAPE'] = "Used escape strategy"
