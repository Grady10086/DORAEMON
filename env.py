...
class Env:
    ...
    def _calculate_metrics(self, agent_state: habitat_sim.AgentState, agent_action: PolarAction, geodesic_path: int, max_steps: int):
        ...
        # 获取智能体的AORI指标
        if hasattr(self.agent, 'area_tracker') and hasattr(self.agent, 'calculate_trustworthy_metrics'):
            trustworthy_data = self.agent.calculate_trustworthy_metrics()
            metrics['trustworthy_metrics'] = trustworthy_data.get("aori", 1.0)  # 默认值为1.0（最差）
            # 添加其他可能需要的详细指标
